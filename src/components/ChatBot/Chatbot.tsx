...
import { ArrowDownTrayIconOutline, XMarkIconOutline, CogIconOutline } from '@neo4j-ndl/react/icons';
import ChatBotAvatar from '../../assets/images/grappus_logo.jfif';
import {
  ChatbotProps,
  Chunk,
  Community,
...
import SystemPromptIndicator from './SystemPromptIndicator';
...
const Chatbot: FC<ChatbotProps> = (props) => {
  ...
  const handleSubmit = async (e: { preventDefault: () => void }) => {
    ...
    try {
      // Combine system prompt with user input if enabled
      const finalMessage =
  ...
  return (
    <div className='flex! flex-col justify-between min-h-full max-h-full overflow-hidden relative'>
      {isDeleteChatLoading && (
      ...
      >
        <Widget className='w-full' header='' isElevated={false}>
              ...
              return (
                ...
                >
                  ...
                  <Widget
                    ...
                    style={{
                      backgroundColor: chat.user === 'chatbot' ? 'hsl(var(--secondary))' : 'rgba(202, 248, 0, 0.15)',
                      borderLeft: chat.user === 'chatbot' ? 'none' : '3px solid #caf800',
                      color: chat.user === 'chatbot' ? 'hsl(var(--secondary-foreground))' : 'hsl(var(--foreground))',
                    }}
                  >
                    <div
                      className={`${
      ...
      {/* <div className='px-5'>
        <SystemPromptIndicator onOpenSettings={() => setShowSystemPromptSettings(true)} />
      </div> */}

      <div
        className='flex! gap-2.5 bottom-0 p-2.5 w-full border-t'
        style={{
          borderColor: 'rgba(202, 248, 0, 0.2)',
        }}
      >
        <form onSubmit={handleSubmit} className={`flex! gap-2.5 w-full ${!isFullScreen ? 'justify-between' : ''}`}>
          <TextInput
            className={`flex-grow-7 ${isFullScreen ? 'w-[calc(100%-105px)]' : 'w-[70%]'}`}
            value={inputMessage}
            isFluid
          ...
          >
            <CogIconOutline className='w-4 h-4' />
          </ButtonWithToolTip>
      ...
      <SystemPromptSettings isOpen={showSystemPromptSettings} onClose={() => setShowSystemPromptSettings(false)} />
    </div>
  );
};
