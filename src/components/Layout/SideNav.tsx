...
const SideNav: React.FC<SideNavProps> = ({
  ...
  const renderDataSourceItems = () => {
    ...
    if (!isLargeDesktop && position === 'right') {
      dataSourceItems.push(
        <SpotlightTarget id='dropzone' indicatorVariant='point' indicatorPlacement='middle-left'>
          <SideNavigation.Item
            key='local'
            icon={
              <TooltipWrapper tooltip='Local Files' placement='left'>
      ...
      if (APP_SOURCES.includes('gcs') && position === 'right') {
        dataSourceItems.push(
          <SideNavigation.Item
            key='gcs'
            icon={
              <TooltipWrapper tooltip='GCS Files' placement='left'>
      ...
      if (APP_SOURCES.includes('s3') && position === 'right') {
        dataSourceItems.push(
          <SideNavigation.Item
            key='s3'
            icon={
              <TooltipWrapper tooltip='S3 Files' placement='left'>
                <S3Component
      ...
      if (APP_SOURCES.includes('web') && position === 'right') {
        dataSourceItems.push(
          <SideNavigation.Item
            key='web'
            icon={
              <TooltipWrapper tooltip='Web Sources' placement='left'>
                <WebButton
                  isLargeDesktop={false}
  ...
  return (
    <div className='sidenav-container' style={{ backgroundColor: '#000000' }}>
      <SideNavigation hasIconMenu={true} isExpanded={false} position={position}>
        <SideNavigation.List>
          ...
          {!isExpanded && position === 'right' && isLargeDesktop && (
            <SideNavigation.Item
              htmlAttributes={{ onClick: handleUploadClick }}
              icon={
                <TooltipWrapper tooltip={tooltips.sources} placement='left'>
